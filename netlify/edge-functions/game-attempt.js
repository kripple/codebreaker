...
const winningFeedback = new Array(config.solutionLength).fill(correct).join("");
...
function evaluateAttempt(guess, solution) {
  ...
  for (let j = 0; j < tokens.length; j++) {
    ...
    const index2 = secretTokens.findIndex(
      (secretToken) => Boolean(token && secretToken) && token === secretToken
    );
    if (index2 !== -1) {
      feedback.push("O");
      delete tokens[j];
      delete secretTokens[index2];
    }
  }
